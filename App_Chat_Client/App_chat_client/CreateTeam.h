...
#define EDIT_TEAMNAME 502
#define BTN_CREATE 503
#define BTN_ADD 504
#define LB_USER_ADDED 505

bool wcCreateGroupRegisteredbefore = false;
LRESULT CALLBACK windowProcessForWindowCreateTeam(HWND handleforwindow1, UINT message, WPARAM wParam, LPARAM lParam);
...
void createwindowCreateTeam(WNDCLASSEX& wc, HWND& hwnd, HINSTANCE hInst, int nShowCmd) {
	if (wcCreateGroupRegisteredbefore == false) {
		...
		wc.hIconSm = NULL;
		wc.hInstance = hInst;
		wc.lpfnWndProc = (WNDPROC)windowProcessForWindowCreateTeam;
	...
	hwnd = CreateWindowEx(NULL, wc.lpszClassName, "Window Create Team", WS_OVERLAPPEDWINDOW, 600, 70, 640, 280,
		NULL, NULL, hInst, NULL);

	if (!hwnd)
	{
		int nResult = GetLastError();
		MessageBox(NULL, "Window Create Team creation failed", "Window Creation Failed", MB_ICONERROR);
	}

	ShowWindow(hwnd, nShowCmd);
}

/*draw window creating team UI*/
void addControlCreateTeam(HWND hwnd) {
	...
	CreateWindowW(L"static", L"Team name: ", WS_VISIBLE | WS_CHILD | WS_BORDER, 300, 100, 200, 25, hwnd,
		NULL, NULL, NULL);
	hEdit_nameTeam = CreateWindowW(L"edit", L"", WS_VISIBLE | WS_CHILD | WS_BORDER, 300, 128, 200, 25, hwnd,
		(HMENU)EDIT_TEAMNAME, NULL, NULL);
